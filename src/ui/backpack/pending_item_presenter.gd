...
static func render(
	...
	for entry in pending_items:
		...
		var safe_area_width: float = maxf(area_size.x - CARD_MARGIN.x * 2.0, 1.0)
		var columns := int(floor(safe_area_width / maxf(card_size.x + CARD_SPACING, 1.0)))
		if columns <= 0:
			columns = 1
		var row: int = index / columns
		var column: int = index % columns
		var row_start: int = row * columns
		var row_items_count: int = mini(columns, pending_items.size() - row_start)
		if row_items_count <= 0:
			row_items_count = 1
		var row_width: float = float(row_items_count) * card_size.x + maxf(0.0, float(row_items_count - 1) * CARD_SPACING)
		var row_x_offset: float = maxf(0.0, (safe_area_width - row_width) * 0.5)
		var x: float = CARD_MARGIN.x + row_x_offset + float(column) * (card_size.x + CARD_SPACING)
		var y: float = CARD_MARGIN.y + float(row) * (card_size.y + CARD_SPACING)
		var safe_area_height: float = maxf(area_size.y - CARD_MARGIN.y * 2.0, 1.0)
