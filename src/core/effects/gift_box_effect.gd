...
func _replace_with_random_small_items(instance: BackpackManager.ItemInstance, backpack: BackpackManager, context: GameContext) -> int:
	...
	var item_db = context.state.get_node_or_null("/root/ItemDatabase") if context and context.state else null
...
func _get_run_manager(context: GameContext) -> Node:
	if context == null or context.state == null or not context.state.is_inside_tree():
		return null
	return context.state.get_node_or_null("/root/RunManager")
