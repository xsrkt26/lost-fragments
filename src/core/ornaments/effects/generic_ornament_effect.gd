...
func after_item_discarded(item_data: ItemData, _old_instance: BackpackManager.ItemInstance, _from_backpack: bool, context: GameContext, state: Dictionary) -> void:
	match effect_id:
		"sanity_coin_purse":
		...
		"fusion_badge":
			for target in targets:
				if _has_same_tag_neighbor(_backpack(context), target):
					_add_score(context, 2)
		"tri_phase_crown":
			if hit_count >= 5 and state.get("pollution_seen", false) and state.get("tool_seen", false) and not state.get("used", false):
				_add_score(context, 45)
				state["used"] = true
...
func _record_universal_toolbox_target(tool_data, target: Dictionary, context: GameContext, state: Dictionary) -> void:
	...
	match target_type:
		...
		"discard":
			state["used_on_discard"] = true
		"dreamcatcher":
			state["used_on_dreamcatcher"] = true
	if state.get("rewarded", false):
		return
	if state.get("used_on_item", false) and state.get("used_on_discard", false) and state.get("used_on_dreamcatcher", false):
