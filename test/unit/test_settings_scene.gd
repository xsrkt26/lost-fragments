...
func test_settings_defaults_start_unmuted() -> void:
	var previous_audio := SettingsManager.audio_settings.duplicate(true)
	...
	assert_false(bool(SettingsManager.audio_settings["is_muted"]))
...
func test_legacy_fullscreen_display_settings_migrate_to_borderless_fullscreen() -> void:
	...
	legacy_config.set_value("display", "window_mode", SettingsManager.WINDOW_MODE_FULLSCREEN)
	legacy_config.set_value("display", "resolution", Vector2i(1920, 1080))
	assert_eq(legacy_config.save(SettingsManager.SETTINGS_FILE), OK)
	SettingsManager.config = ConfigFile.new()
