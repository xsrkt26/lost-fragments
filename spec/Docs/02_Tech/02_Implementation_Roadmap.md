...
- 默认路线为 5 个节点：局内游戏、商店、局内游戏、商店、Boss 局内游戏。
- 第 2-6 层已有独立主题路线，保持 5 节点与第 5 节点 Boss 的通关节奏。
- 游戏共 6 个场景层，完成第 6 层路线后整局胜利。
- Hub 由当前路线节点驱动，`Z` 键可进入当前战斗或商店节点。
- 普通战斗默认无分数目标，Boss 战有目标分数；Boss 达标会自动请求结束，梦值归零或手动结束时统一判定胜负。
...
- 事件系统保留权重随机、去重、节点缓存、事务回滚和高风险背包效果能力，但当前主路线不再投放事件节点。
